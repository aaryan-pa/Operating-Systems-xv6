  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        // cprintf("Process end time = %d\n",curproc->end_time);
        // cprintf("Time consumed by process = %d\n",curproc->ran_time);
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
        p->pid = 0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      // if(current == 0 || p->priority < current->priority)
      // current = p;
...
//   acquire(&ptable.lock);
//  cprintf("pid \t\t state \t\t start_time \t\t total_time \t name \n");
// for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
//   if(strncmp(p->name, pr_name,99) == 0)
// 	  {
//       if(p->state == SLEEPING)
