    ...
    334c:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    3379:	0f b6 92 78 37 00 00 	movzbl 0x3778(%edx),%edx
    ...
    33f9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    3469:	ff 24 85 20 37 00 00 	jmp    *0x3720(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    3577:	90                   	nop
          s = "(null)";
    3578:	ba 18 37 00 00       	mov    $0x3718,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    3591:	a1 20 3a 00 00       	mov    0x3a20,%eax
{
  ...
  freep = p;
    ...
    35d1:	5b                   	pop    %ebx
  freep = p;
    35d2:	89 15 20 3a 00 00    	mov    %edx,0x3a20
}
    35d8:	5e                   	pop    %esi
    35d9:	5f                   	pop    %edi
    35da:	5d                   	pop    %ebp
  ...
  freep = p;
    360c:	89 15 20 3a 00 00    	mov    %edx,0x3a20
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    362c:	8b 3d 20 3a 00 00    	mov    0x3a20,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    3671:	8b 3d 20 3a 00 00    	mov    0x3a20,%edi
  ...
  return freep;
    369f:	8b 15 20 3a 00 00    	mov    0x3a20,%edx
    ...
    36cc:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    36cf:	89 15 20 3a 00 00    	mov    %edx,0x3a20
}
    ...
    36df:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    36e0:	c7 05 20 3a 00 00 24 	movl   $0x3a24,0x3a20
    36e7:	3a 00 00 
    base.s.size = 0;
    36ea:	bf 24 3a 00 00       	mov    $0x3a24,%edi
    base.s.ptr = freep = prevp = &base;
    36ef:	c7 05 24 3a 00 00 24 	movl   $0x3a24,0x3a24
    36f6:	3a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    36f9:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    36fb:	c7 05 28 3a 00 00 00 	movl   $0x0,0x3a28
    3702:	00 00 00 
    if(p->s.size >= nunits){
